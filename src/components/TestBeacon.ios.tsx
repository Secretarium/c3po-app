...
export default class IBeaconExample extends Component {
    ...
    startMonitoringSubscription: any = null;
    monitoringFailSubscription: any = null;
    regionEnterSubscription: any = null;
    regionExitSubscription: any = null;
    regionRangeSubscription: any = null;
    regionRangeFailSubscription: any = null;
    authorizationSubscription: any = null;
    discoverSubscription: any = null;
    discoverFailSubscription: any = null;
    ...
    componentDidMount() {
        ...
        this.regionExitSubscription = kontaktEmitter.addListener(
            ...
            ({ region: exitRegion }) => {
                ...
                const index = monitoredRegionsCloseBy.findIndex((region: any) =>
                    this._isIdenticalRegion(exitRegion, region)
                );
                this.setState({
                    monitoredRegionsCloseBy: monitoredRegionsCloseBy.reduce((result: any, val: any, ind: number) => {
    ...
    _startDiscovery = () => {
        startDiscovery({ interval: 1000 })
            ...
            .catch((error: any) => console.log('[startDiscovery]', error));
    };
    _stopDiscovery = () => {
        stopDiscovery()
            .then(() => this.setState({ scanning: false, discoveredBeacons: [] }))
            .then(() => console.log('stopped discovery'))
            .catch((error: any) => console.log('[stopDiscovery]', error));
    };
    _restartDiscovery = () => {
        restartDiscovery()
            .then(() => this.setState({ scanning: true, discoveredBeacons: [] }))
            .then(() => console.log('restarted discovery'))
            .catch((error: any) => console.log('[restartDiscovery]', error));
    };
    ...
    _startMonitoring = () => {
        startMonitoringForRegion(region1)
            ...
            .catch((error: any) => console.log('[startMonitoring]', error));
    };
    _stopMonitoring = () => {
    ...
    _stopAllMonitoring = () => {
        stopMonitoringForAllRegions()
            ...
            .catch((error: any) => console.log('[stopAllMonitoring]', error));
    };
    ...
    _getRangedRegions = () => {
        getRangedRegions()
            .then((regions: any) => this.setState({ rangedRegions: regions }))
            .then(() => console.log('got ranged regions'))
            .catch((error: any) => console.log('[getRangedRegions]', error));
    };

    _getMonitoredRegions = () => {
        getMonitoredRegions()
            .then((regions: any) => this.setState({ monitoredRegions: regions }))
            .then(() => console.log('got monitored regions'))
            .catch((error: any) => console.log('[getMonitoredRegions]', error));
    };

    /**
     * Helper function used to identify equal regions
     */
    _isIdenticalRegion = (r1: { identifier: any; }, r2: { identifier: any; }) => (
    ...
    _renderDiscoveredBeacons = () => {
        ...
        return this.state.discoveredBeacons.sort((a: any, b: any) => a.accuracy - b.accuracy).map((beacon: any, ind: number) => (
    ...
    _renderRangedBeacons = () => {
        ...
        return this.state.rangedBeacons.sort((a: any, b: any) => a.accuracy - b.accuracy).map((beacon: any, ind: number) => (
            <View key={ind} style={[styles.beacon, { backgroundColor: colors[beacon.minor - 1] }]}>
    ...
    _renderMonitoredRegions = () => {
        ...
        return this.state.monitoredRegionsCloseBy.map((region: any, ind: number) => (
        ...
        return (
            <View>
                <Text style={{ color: '#ABE88D' }}>
                    {rangedRegions !== [] ? rangedRegions.reduce((result: any, region: any) => result + region.identifier + ', ', '') : 'No ranged regions'}
                </Text>
                <Text style={{ color: '#F48661' }}>
                    {monitoredRegions !== [] ? monitoredRegions.reduce((result: any, region: any) => result + region.identifier + ', ', '') : 'No monitored regions'}
                </Text>
            </View>
        );
    };
    ...
    _renderButton = (text: React.ReactNode, onPress: ((event: import("react-native").GestureResponderEvent) => void) | undefined, backgroundColor: any) => (
